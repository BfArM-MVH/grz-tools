...
target_qc_percentage = snakemake.params.target_qc_percentage
...
with redirect_stdout(open(stdout_log_path, "w")), redirect_stderr(open(stderr_log_path, "w")):
    ...
    with open(output_batch_file, "w") as f:
        for submission in selected_submissions:
            target_path = shared.determine_target_path(
                submission["origin"], submission["submission_id"], target_qc_percentage, db_config_path
            )
