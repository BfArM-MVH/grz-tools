...
GENOMIC_DATA_CENTER_ID_PATTERN = r"^GRZ[A-Z0-9]{3}[0-9]{3}$"
SUBMITTER_ID_PATTERN = r"^[0-9]{9}$"


def valid_test_tan(v: str) -> str:
    if len(v) != 64:
        raise ValueError("Test TANs must be 64 characters")
    if v[:4] != "TEST":
        raise ValueError("Test TANs must begin with 'TEST'")
    if not re.fullmatch(GENOMIC_DATA_CENTER_ID_PATTERN, v[4:13]):
        raise ValueError("Test TANs must have the GRZ ID after 'TEST'")
    if not re.fullmatch(SUBMITTER_ID_PATTERN, v[13:22]):
        raise ValueError("Test TANs must have the submitter ID after the GRZ ID")
    if not re.fullmatch(r"[A-Za-z0-9]{42}", v[22:]):
        raise ValueError("Test TANs must end with 42 alphanumeric characters")

    return v


Tan = Annotated[str, StringConstraints(pattern=r"^[A-Fa-f0-9]{64}$")]
TestTan = Annotated[str, AfterValidator(valid_test_tan)]
TestTanValidator = TypeAdapter(TestTan)
SubmitterId = Annotated[str, StringConstraints(pattern=SUBMITTER_ID_PATTERN)]
GenomicDataCenterId = Annotated[str, StringConstraints(pattern=GENOMIC_DATA_CENTER_ID_PATTERN)]
...
class Submission(StrictBaseModel):
    ...
    def ensure_valid_test_submission(self):
        is_test_tan = False
        try:
            # unfortunately cannot check isinstance on Annotated str so we must try to validate
            TestTanValidator.validate_python(self.tan_g)
            is_test_tan = True
        except ValueError:
            # assume if we got this far that ValueError means we have a real TAN, not a test TAN
            pass

        if is_test_tan:
            if self.submission_date != date(year=2001, month=1, day=1):
                raise ValueError("Test submissions must have a submission date of 01.01.2001")
            if self.genomic_data_center_id != self.tan_g[4:13]:
                raise ValueError("GRZ ID embedded within test TAN must match the GRZ ID in the submission metadata")
            if self.submitter_id != self.tan_g[13:22]:
                raise ValueError(
                    "Submitter ID embedded within test TAN must match submitter ID in the submission metadata"
                )
