...
def full_config_path(
    ...
    # flag actually selects the intended archive target.
    archives["consented"]["public_key_path"] = CONSENTED_PUBLIC_KEY_PATH
    archives["non_consented"]["public_key_path"] = NON_CONSENTED_PUBLIC_KEY_PATH
...
def mock_command(command_spec, submission_id):
    ...
    with contextlib.ExitStack() as stack:
        ...
        if "worker_patch" in command_spec:
            ...
            # decrypt and encrypt load the private key of this submitter's inbox, which full_config_path configures
...
@pytest.mark.parametrize(
    ...
    [
        {
            "cmd": ["download", "--submission-id", "SUBMISSION_ID", "--output-dir", "OUTPUT_DIR", "--inbox", "inbox"],
...
def test_validation_basic_qc_passed_update(
    ...
    with (
        patch("grzctl.commands.validate.Worker") as mock_worker_cls,
    ):
        mock_worker = mock_worker_cls.return_value
